...
func ScreenRect() (*rectangle.Rectangle, error) {
	hDC := GetDC(0)
	if hDC == 0 {
	...
	return rectangle.New(0, 0, x, y), nil
}
...
func Capture(r *rectangle.Rectangle) (image.Image, error) {
	rect := rectangle.RectangleToImageRectangle(r)
	hDC := GetDC(0)
	if hDC == 0 {
		return nil, fmt.Errorf("Could not Get primary display err:%d.\n", GetLastError())
	}
...
var (
	...
	vkKeyScan 			   = moduser32.NewProc("VkKeyScanEx")
